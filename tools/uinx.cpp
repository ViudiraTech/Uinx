...
std::optional<ast::SmpMode> parse_smp_mode(std::string_view value) {
    if (value == "auto")
        return ast::SmpMode::Auto;
    if (value == "manual")
        return ast::SmpMode::Manual;
    if (value == "strict")
        return ast::SmpMode::Strict;
    return std::nullopt;
}
...
std::optional<KernelTarget> kernel_target(std::string_view arch) {
    if (arch == "x86_64")
        return KernelTarget{"x86_64-unknown-none", "elf_x86_64", "hlt"};
    if (arch == "aarch64")
        return KernelTarget{"aarch64-unknown-none", "aarch64linux", "wfe"};
    if (arch == "riscv64")
        return KernelTarget{"riscv64-unknown-none-elf", "elf64lriscv", "wfi"};
    return std::nullopt;
}
...
int build_kernel(const std::filesystem::path& root,
                 const Manifest& manifest,
                 bool release,
    ...
    assemble << "clang --target=" << target->triple << " -c " << shell_quote(startup) << " -o "
             << shell_quote(startup_object);
    if (run_shell(assemble.str()) != 0)
        return 1;
...
int main(int argc, char** argv) {
    ...
    if (command == "new") {
        if (argc < 3) {
            std::cerr
                << "uinx new <name> [--lib] [--freestanding] [--kernel[=x86_64|aarch64|riscv64]]\n";
            return 2;
        }
        bool library = false;
        ...
        for (int i = 3; i < argc; ++i) {
            ...
            if (arg == "--lib")
                library = true;
            else if (arg == "--freestanding" || arg == "--no-std")
                freestanding = true;
            else if (arg == "--kernel")
                kernel_arch = "x86_64";
            else if (arg.rfind("--kernel=", 0) == 0)
                kernel_arch = arg.substr(9);
        }
        if (!kernel_arch.empty()) {
            if (library || !kernel_target(kernel_arch)) {
                std::cerr << "--kernel requires x86_64, aarch64, or riscv64 and cannot be combined "
                             "with --lib\n";
                return 2;
            }
            freestanding = true;
        ...
        if (!kernel_arch.empty())
            manifest_file << "arch = \"" << kernel_arch << "\"\n";
        manifest_file << "\n[dependencies]\n";
