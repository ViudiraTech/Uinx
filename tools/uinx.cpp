...
std::optional<ast::SmpMode> parse_smp_mode(std::string_view value) {
  if (value == "auto") return ast::SmpMode::Auto;
  if (value == "manual") return ast::SmpMode::Manual;
  if (value == "strict") return ast::SmpMode::Strict;
  return std::nullopt;
}
...
SourceSelection source_selection(const std::filesystem::path& root, const Manifest& manifest) {
  ...
  while (std::getline(source, line)) {
    ...
    if (name == "std") selection.std_mode = disabled ? "none" : "full";
    else if (!disabled && (name == "core" || name == "alloc" || name == "minimal"))
      selection.std_mode = name;
    else if (name == "runtime") selection.runtime = !disabled;
    else if (disabled && !name.empty()) selection.disabled_dependencies.insert(name);
  }
  return selection;
}
...
bool read_manifest(const std::filesystem::path& path, Manifest& manifest) {
  ...
  while (std::getline(file, line)) {
    ...
    if (section == "package") {
      ...
      else if (key == "version") manifest.version = unquote(value);
      else if (key == "entry") manifest.entry = unquote(value);
      else if (key == "kind") manifest.kind = unquote(value);
      else if (key == "std") manifest.std_mode = unquote(value);
      else if (key == "arch") manifest.arch = unquote(value);
...
std::filesystem::path find_root() {
  ...
  for (;;) {
    if (std::filesystem::exists(path / "uinx.toml")) return path;
...
std::vector<std::filesystem::path> ux_files_under(const std::filesystem::path& directory) {
  ...
  for (const auto& entry : std::filesystem::recursive_directory_iterator(directory)) {
    if (entry.is_regular_file() && entry.path().extension() == ".ux") files.push_back(entry.path());
  }
...
std::filesystem::path stdlib_root() {
  ...
  if (const char* env = std::getenv("UINX_STDLIB")) candidates.emplace_back(env);
...
std::filesystem::path runtime_library() {
  ...
  if (const char* env = std::getenv("UINX_RUNTIME_LIB")) candidates.emplace_back(env);
...
void append_stdlib_sources(std::vector<std::filesystem::path>& out, std::string_view mode) {
  if (mode == "none" || mode == "no_std") return;
  ...
  if (mode == "core") return;
  append_layer("alloc");
  if (mode == "alloc") return;
  append_layer("minimal");
  if (mode == "minimal") return;
  append_layer("std");
}

bool collect_dependency_sources(const std::filesystem::path& root, const Manifest& manifest,
                                std::vector<std::filesystem::path>& out,
                                std::set<std::filesystem::path>& visited, std::string& error,
                                const std::set<std::string>* disabled_dependencies = nullptr) {
  for (const auto& [name, relative] : manifest.path_dependencies) {
    if (disabled_dependencies && disabled_dependencies->contains(name)) continue;
    std::error_code ec;
    ...
    if (!visited.insert(dep_root).second) continue;
    Manifest dep;
    ...
    if (!collect_dependency_sources(dep_root, dep, out, visited, error, nullptr)) return false;
  }
  return true;
}

bool collect_project_sources(const std::filesystem::path& root, const Manifest& manifest,
                             std::vector<std::filesystem::path>& out, bool include_std = true) {
  out.clear();
  ...
  for (const auto& source : local) if (std::filesystem::weakly_canonical(source) != std::filesystem::weakly_canonical(entry)) out.push_back(source);
  ...
  std::string error;
  if (!collect_dependency_sources(root, manifest, out, visited, error, &selection.disabled_dependencies)) {
    std::cerr << error << '\n';
    return false;
  }
  if (include_std) append_stdlib_sources(out, selection.std_mode);
  return true;
}
...
int fetch(const std::filesystem::path& root, const Manifest& manifest) {
  ...
  write_deps = [&](const std::filesystem::path& owner_root, const Manifest& owner) {
    for (const auto& [name, relative] : owner.path_dependencies) {
      ...
      if (ec || !visited.insert(dep_root).second) continue;
      ...
      contents << file.rdbuf();
      Manifest dependency;
      if (!read_manifest(manifest_path, dependency)) return false;
      lock << "\n[[package]]\nname = \"" << name << "\"\nversion = \"" << dependency.version
           << "\"\nsource = \"path+" << dep_root.string() << "\"\nchecksum = \"" << std::hex << fnv1a(contents.str())
           << std::dec << "\"\n";
      if (!write_deps(dep_root, dependency)) return false;
    }
    return true;
  };
  if (!write_deps(root, manifest)) return 1;
...
struct KernelTarget {
  std::string triple;
  std::string linker_emulation;
  std::string wait_instruction;
};

std::optional<KernelTarget> kernel_target(std::string_view arch) {
  if (arch == "x86_64") return KernelTarget{"x86_64-unknown-none", "elf_x86_64", "hlt"};
  if (arch == "aarch64") return KernelTarget{"aarch64-unknown-none", "aarch64linux", "wfe"};
  if (arch == "riscv64") return KernelTarget{"riscv64-unknown-none-elf", "elf64lriscv", "wfi"};
  return std::nullopt;
}
...
int run_shell(std::string command) {
  const int status = std::system(command.c_str());
  if (status != 0) std::cerr << "command failed: " << command << '\n';
  return status;
}

int build_kernel(const std::filesystem::path& root, const Manifest& manifest, bool release,
  ...
  if (!collect_project_sources(root, manifest, sources)) return 1;
  const auto directory = root / "target" / (release ? "release" : "debug");
  std::filesystem::create_directories(directory);
  const auto kernel_object = directory / (manifest.name + "-kernel.o");
  const auto startup_object = directory / (manifest.name + "-start.o");
  const auto executable = directory / (manifest.name + ".elf");
  const auto startup = root / "arch" / manifest.arch / "start.S";
  const auto linker = root / "arch" / manifest.arch / "link.ld";
  if (!std::filesystem::exists(startup) || !std::filesystem::exists(linker)) {
    std::cerr << "kernel package is missing " << startup << " or " << linker << '\n';
    return 1;
  }
  ...
  if (manifest.arch == "riscv64") options.code_model = "medany";
  options.output = kernel_object;
  Compiler compiler(&std::cerr);
  const auto result = compiler.compile_files(sources, options);
  if (!result.success) return 1;

  std::ostringstream assemble;
  assemble << "clang --target=" << target->triple << " -c " << shell_quote(startup)
           << " -o " << shell_quote(startup_object);
  if (run_shell(assemble.str()) != 0) return 1;

  std::ostringstream link;
  link << "ld.lld -m " << target->linker_emulation << " -T " << shell_quote(linker) << ' '
       << shell_quote(startup_object) << ' ' << shell_quote(kernel_object) << " -o "
       << shell_quote(executable);
  if (run_shell(link.str()) != 0) return 1;
  if (output_path) *output_path = executable;
  std::cout << executable << '\n';
  return 0;
}

int build_project(const std::filesystem::path& root, const Manifest& manifest, EmitKind emit,
                  bool release, std::filesystem::path* output_path = nullptr,
                  std::optional<ast::SmpMode> smp_override = std::nullopt) {
  std::vector<std::filesystem::path> sources;
  if (!collect_project_sources(root, manifest, sources)) return 1;
  CompileOptions options;
  ...
  const auto directory = root / "target" / (release ? "release" : "debug");
  std::filesystem::create_directories(directory);
  if (emit == EmitKind::Executable) options.output = directory / manifest.name;
  else if (emit == EmitKind::Object) options.output = directory / (manifest.name + ".o");
  const auto selection = source_selection(root, manifest);
  if (emit == EmitKind::Executable && selection.runtime && selection.std_mode != "none" &&
    ...
    if (!runtime.empty()) options.linker_args.push_back(runtime.string());
  }
  Compiler compiler(&std::cerr);
  auto result = compiler.compile_files(sources, options);
  if (output_path) *output_path = result.output;
...
int format_file(const std::filesystem::path& path, bool check) {
  ...
  while (std::getline(input, line)) {
    ...
    std::size_t width = 0;
    for (std::size_t i = 0; i < first; ++i) width += line[i] == '\t' ? 4u : 1u;
    while (indentation.size() > 1 && width < indentation.back()) indentation.pop_back();
    if (width > indentation.back()) indentation.push_back(width);
...
int generate_docs(const std::vector<std::filesystem::path>& sources, const std::filesystem::path& output_path) {
  ...
  for (const auto& source_path : sources) {
    ...
    for (const auto& item : module.items) {
      if (auto function = std::get_if<ast::FunctionDecl>(&item)) {
        ...
        if (!structure->is_pub) continue;
        if (!wrote_file) {
          output << "## " << source_path.filename().string() << "\n\n";
          wrote_file = true;
        }
        output << "### `struct " << structure->name << "`\n\n";
        for (const auto& field : structure->fields) output << "- `" << field.name << ": " << field.type.str() << "`\n";
        output << '\n';
      } else if (auto trait = std::get_if<ast::TraitDecl>(&item)) {
        if (!trait->is_pub) continue;
        if (!wrote_file) {
          output << "## " << source_path.filename().string() << "\n\n";
          wrote_file = true;
        }
        output << "### `trait " << trait->name << "`\n\n";
        for (const auto& method : trait->methods) output << "- `" << method.name << "(...) -> " << method.return_type.str() << "`\n";
        output << '\n';
      }
    }
  }
...
int main(int argc, char** argv) {
  ...
  if (command == "new") {
    if (argc < 3) {
      std::cerr << "uinx new <name> [--lib] [--freestanding] [--kernel[=x86_64|aarch64|riscv64]]\n";
      return 2;
    }
    bool library = false;
    ...
    for (int i = 3; i < argc; ++i) {
      ...
      if (arg == "--lib") library = true;
      else if (arg == "--freestanding" || arg == "--no-std") freestanding = true;
      else if (arg == "--kernel") kernel_arch = "x86_64";
      else if (arg.rfind("--kernel=", 0) == 0) kernel_arch = arg.substr(9);
    }
    if (!kernel_arch.empty()) {
      if (library || !kernel_target(kernel_arch)) {
        std::cerr << "--kernel requires x86_64, aarch64, or riscv64 and cannot be combined with --lib\n";
        return 2;
      }
      freestanding = true;
    }
    ...
    std::ofstream manifest_file(root / "uinx.toml");
    manifest_file << "[package]\nname = \"" << root.filename().string()
                  << "\"\nversion = \"0.1.0\"\nentry = \"" << entry << "\"\nkind = \""
                  << (!kernel_arch.empty() ? "kernel" : (library ? "lib" : "bin"))
                  << "\"\nstd = \"" << (freestanding ? "none" : "full") << "\"\n";
    if (!kernel_arch.empty()) manifest_file << "arch = \"" << kernel_arch << "\"\n";
    manifest_file << "\n[dependencies]\n";

    std::ofstream source(root / entry);
    if (freestanding) source << "dontneed std\n";
    if (!kernel_arch.empty()) {
      source << "need core\nsmp auto\n\n"
             ...
             << "    online_cpus += 1\n"
             << "    return\n";
      const auto arch_dir = root / "arch" / kernel_arch;
      std::filesystem::create_directories(arch_dir);
      std::ofstream startup(arch_dir / "start.S");
      std::ofstream linker(arch_dir / "link.ld");
      ...
      source << "\npublic func library_ready() -> bool:\n    return true\n";
    } else {
      if (freestanding) source << '\n';
  ...
  for (int i = 2; i < argc; ++i) {
    ...
    if (arg == "--release") release = true;
      ...
      if (!smp_override) {
        std::cerr << "invalid --smp mode (expected auto, manual, or strict)\n";
        return 2;
      }
    }
  }

  if (command == "fetch") return fetch(root, manifest);
  if (command == "build") {
    if (manifest.kind == "kernel") return build_kernel(root, manifest, release, nullptr, smp_override);
    return build_project(root, manifest, manifest.kind == "lib" ? EmitKind::Object : EmitKind::Executable, release, nullptr, smp_override);
  }
  if (command == "check") return build_project(root, manifest, EmitKind::Check, release, nullptr, smp_override);
  if (command == "run") {
    ...
    const int result = build_project(root, manifest, EmitKind::Executable, release, &executable, smp_override);
  ...
  if (command == "test") {
    ...
    if (!collect_project_sources(root, manifest, base)) return 1;
    base.erase(std::remove_if(base.begin(), base.end(), [&](const auto& source) {
                 return std::filesystem::weakly_canonical(source) == std::filesystem::weakly_canonical(root / manifest.entry) && manifest.kind == "bin";
               }), base.end());
    ...
    for (const auto& test : tests) {
      ...
      Compiler compiler(&std::cerr);
      const auto result = compiler.compile_files(sources, options);
      if (!result.success || std::system(("\"" + options.output.string() + "\"").c_str()) != 0) ++failed;
    }
    std::cout << (static_cast<int>(tests.size()) - failed) << "/" << tests.size() << " tests passed\n";
    return failed ? 1 : 0;
  }
  if (command == "fmt") {
    bool check = false;
    for (int i = 2; i < argc; ++i) if (std::string(argv[i]) == "--check") check = true;
    int failed = 0;
    for (const auto& source : ux_files_under(root / "src")) failed += format_file(source, check) != 0;
    return failed ? 1 : 0;
  }
  if (command == "lint") {
    std::vector<std::filesystem::path> sources;
    ...
    Compiler compiler(&std::cerr);
    const auto result = compiler.compile_files(sources, options);
    if (result.success) std::cout << "lint: semantic, type, ownership and borrow checks passed\n";
    return result.success ? 0 : 1;
  }
  if (command == "doc") {
    std::vector<std::filesystem::path> sources;
    if (!collect_project_sources(root, manifest, sources, false)) return 1;
    const auto directory = root / "target/doc";
    std::filesystem::create_directories(directory);
    const auto output = directory / "api.md";
    const int result = generate_docs(sources, output);
    if (!result) std::cout << output << '\n';
    return result;
  }
  if (command == "add") {
