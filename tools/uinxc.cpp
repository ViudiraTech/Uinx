...
void usage() {
  std::cout << "uinxc <file.ux> [more.ux ...] [-o output] [--emit=check|llvm-ir|obj|exe] "
               "[-O0..3] [--target=TRIPLE] [--clang=PATH] [--smp=auto|manual|strict] [--freestanding] [--keep-temps] [--no-verify-ir]\n";
}
}  // namespace
...
int main(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg.rfind("--emit=", 0) == 0) {
      ...
      if (value == "check") options.emit = EmitKind::Check;
      else if (value == "llvm-ir") options.emit = EmitKind::LLVMIR;
      else if (value == "obj") options.emit = EmitKind::Object;
      else if (value == "exe") options.emit = EmitKind::Executable;
      else {
    ...
    if (arg.rfind("--smp=", 0) == 0) {
      ...
      if (value == "auto") options.smp_mode_override = ast::SmpMode::Auto;
      else if (value == "manual") options.smp_mode_override = ast::SmpMode::Manual;
      else if (value == "strict") options.smp_mode_override = ast::SmpMode::Strict;
      else {
  ...
  if (options.emit == EmitKind::Check) options.output.clear();
  Compiler compiler(&std::cerr);
  const auto result = compiler.compile_files(inputs, options);
  if (!result.success) return 1;
  if (options.emit != EmitKind::Check) std::cout << result.output.string() << '\n';
  return 0;
}
