...
struct CompileOptions {
  ...
  std::optional<ast::SmpMode> smp_mode_override;
...
class Compiler {
 public:
  ...
  CompileResult compile_files(const std::vector<std::filesystem::path>& paths,const CompileOptions& options);
  CompileResult compile_source(std::string file,std::string source,const CompileOptions& options);
  static std::string detect_host_triple(std::string_view clang="clang");
 private:
  CompileResult compile_module(ast::Module module,std::string logical_file,const CompileOptions& options);
  bool backend_compile(const std::string& ir,const CompileOptions& options,const std::filesystem::path& output);
