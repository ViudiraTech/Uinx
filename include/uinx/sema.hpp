...
class TypeChecker {
  public:
    TypeChecker(Diagnostics& d, hir::Module hir) : diags_(d) {
        model_.hir = std::move(hir);
    }
  ...
  private:
    struct FnContext {
        ...
        std::vector<std::unordered_set<std::string>> mutable_scopes;
    ...
    bool unify_generic(const Type& formal,
                       const Type& actual,
