...
struct Instruction {
    ...
    SourceRange range{};
    Type auxiliary_type;
    Instruction(Op o = Op::Alloca,
                std::string r = {},
                Type t = {},
                std::vector<std::string> a = {},
                std::string tx = {},
                std::vector<AsmOperand> ao = {},
                bool f = false,
                SourceRange sr = {},
                Type aux = {})
        : op(o), result(std::move(r)), type(std::move(t)), args(std::move(a)), text(std::move(tx)),
          asm_operands(std::move(ao)), flag(f), range(std::move(sr)),
          auxiliary_type(std::move(aux)) {
    }
};
struct BasicBlock {
    std::string label;
...
struct Module {
    ...
    std::vector<const ast::StructDecl*> structs;
};
} // namespace uinx::mir
namespace uinx {
class MIRLowerer {
