...
class BorrowChecker {
 ...
 private:
  struct Borrow { std::string place; std::string borrower; bool mut{false}; std::size_t end_stmt{}; SourceRange origin{}; };
  struct RefOrigin { std::string place; SourceRange range{}; };
  struct State {
  ...
  void collect_uses(const ast::Stmt& stmt,std::size_t index,std::unordered_map<std::string,std::size_t>& out) const;
  void collect_expr_uses(const ast::Expr& expr,std::size_t index,std::unordered_map<std::string,std::size_t>& out) const;
  ...
  void access_place(const std::string& place,const SourceRange& range,Access access,State& state);
  void begin_borrow(const std::string& place,std::string borrower,bool mut,std::size_t end,const SourceRange& range,State& state);
  Diagnostics& diags_;
