...
#include <errno.h>
int32_t uinx_utf8_next(const uint8_t* d, size_t n, size_t* off, uint32_t* cp) {
    if (!d || !off || !cp || *off >= n)
        return -EINVAL;
    size_t i = *off;
    uint8_t a = d[i++];
...
int32_t uinx_utf8_validate(const uint8_t* d, size_t n) {
    size_t o = 0;
    uint32_t c;
    while (o < n)
        if (uinx_utf8_next(d, n, &o, &c))
            return -EILSEQ;
    return 0;
}
int32_t uinx_utf8_encode(uint32_t c, uint8_t out[4], size_t* n) {
    if (!out || !n || c > 0x10FFFF || (c >= 0xD800 && c <= 0xDFFF))
        return -EINVAL;
    if (c < 0x80) {
        out[0] = (uint8_t)c;
        *n = 1;
