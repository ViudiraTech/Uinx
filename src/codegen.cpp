        ...
        for (const auto& instruction : block.instructions) {
            switch (instruction.op) {
                ...
                case mir::Op::Load: {
                    ...
                    os << "  " << result << " = load " << llvm_type(instruction.type) << ", ptr "
                       << address << "\n";
                ...
                case mir::Op::Store: {
                    ...
                    os << "  store " << llvm_type(instruction.type) << ' ' << value << ", ptr "
                       << address << "\n";
                    break;
                }
    ...
    for (const auto& block : fn.blocks) {
        ...
        for (const auto& instruction : block.instructions) {
            ...
            switch (instruction.op) {
                ...
                case mir::Op::Load:
                    os << "  " << instruction.result << " = load " << llvm_type(instruction.type)
                       << ", ptr " << value_of(instruction.args[0]) << "\n";
                    values[instruction.result] = instruction.result;
                    value_types[instruction.result] = instruction.type;
                    break;
                case mir::Op::Store:
                    os << "  store " << llvm_type(instruction.type) << ' '
                       << value_of(instruction.args[0]) << ", ptr " << value_of(instruction.args[1])
                       << "\n";
                    break;
                case mir::Op::FieldAddr:
