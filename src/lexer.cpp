...
#define UINX_TOKEN_CASE(name) \
    ...
    switch (kind) {
        ...
        UINX_TOKEN_CASE(KwBreak);
        UINX_TOKEN_CASE(KwContinue);
        UINX_TOKEN_CASE(KwLoop);
        UINX_TOKEN_CASE(KwShared);
        UINX_TOKEN_CASE(KwPerCpu);
        UINX_TOKEN_CASE(KwConcurrent);
        UINX_TOKEN_CASE(KwSmp);
        UINX_TOKEN_CASE(KwAuto);
        UINX_TOKEN_CASE(KwManual);
        UINX_TOKEN_CASE(KwStrict);
        UINX_TOKEN_CASE(KwFence);
        UINX_TOKEN_CASE(KwCompilerFence);
        ...
        UINX_TOKEN_CASE(DotDot);
        UINX_TOKEN_CASE(DotDotEq);
        UINX_TOKEN_CASE(ShiftLeft);
        UINX_TOKEN_CASE(ShiftRight);
        UINX_TOKEN_CASE(AmpEq);
        UINX_TOKEN_CASE(PipeEq);
        UINX_TOKEN_CASE(CaretEq);
        UINX_TOKEN_CASE(PercentEq);
        UINX_TOKEN_CASE(ShiftLeftEq);
        UINX_TOKEN_CASE(ShiftRightEq);
    }
...
Token Lexer::make(TokenKind kind, std::size_t start, SourceLoc begin, std::string text) {
    if (text.empty() && pos_ > start) text = std::string(src_.substr(start, pos_ - start));
...
bool Lexer::skip_comment() {
    ...
    if (depth != 0) diags_.error({begin, loc_}, "E0001", "unterminated block comment");
    return true;
}
...
bool Lexer::consume_blank_or_comment_line() {
    ...
    while (peek() == ' ' || peek() == '\t' || peek() == '\r') advance();
    if (peek() == '#' || (peek() == '/' && peek(1) == '/')) skip_comment();
...
void Lexer::emit_indentation(std::vector<Token>& out) {
    if (!line_start_ || delimiter_depth_ != 0 || eof()) return;
    if (consume_blank_or_comment_line()) return;
...
void Lexer::emit_newline(std::vector<Token>& out, SourceLoc begin) {
    if (delimiter_depth_ != 0) return;
    if (!out.empty() && out.back().kind != TokenKind::Newline && out.back().kind != TokenKind::Indent &&
        out.back().kind != TokenKind::Dedent) {
...
Token Lexer::identifier_or_keyword() {
    ...
    static const std::unordered_map<std::string, TokenKind> keywords = {
        ...
        {"extend", TokenKind::KwImpl},    {"with", TokenKind::KwWith},
        {"pub", TokenKind::KwPub},        {"public", TokenKind::KwPub},
        {"unsafe", TokenKind::KwUnsafe},  {"async", TokenKind::KwAsync},
        {"await", TokenKind::KwAwait},    {"extern", TokenKind::KwExtern},
        {"const", TokenKind::KwConst},    {"static", TokenKind::KwStatic},
        {"move", TokenKind::KwMove},      {"where", TokenKind::KwWhere},
        {"as", TokenKind::KwAs},          {"true", TokenKind::KwTrue},
        {"false", TokenKind::KwFalse},    {"need", TokenKind::KwNeed},
        {"dontneed", TokenKind::KwDontNeed},
        {"no_std", TokenKind::KwDontNeed},
        {"use", TokenKind::KwUse},        {"mod", TokenKind::KwMod},
        {"self", TokenKind::KwSelf},      {"Self", TokenKind::KwSelfTy},
        {"dyn", TokenKind::KwDyn},        {"asm", TokenKind::KwAsm},
        {"ref", TokenKind::KwRef},        {"mutref", TokenKind::KwMutRef},
        {"ptr", TokenKind::KwPtr},        {"mutptr", TokenKind::KwMutPtr},
        {"borrow", TokenKind::KwBorrow},  {"deref", TokenKind::KwDeref},
        {"new", TokenKind::KwNew},        {"scope", TokenKind::KwScope},
        {"pass", TokenKind::KwPass},        {"break", TokenKind::KwBreak},
        {"continue", TokenKind::KwContinue},{"loop", TokenKind::KwLoop},
        {"shared", TokenKind::KwShared},    {"percpu", TokenKind::KwPerCpu},
        {"concurrent", TokenKind::KwConcurrent},
        {"smp", TokenKind::KwSmp},          {"auto", TokenKind::KwAuto},
        {"manual", TokenKind::KwManual},    {"strict", TokenKind::KwStrict},
        {"fence", TokenKind::KwFence},       {"compiler_fence", TokenKind::KwCompilerFence},
    };

    if (text == "and") return make(TokenKind::AndAnd, start, begin, "&&");
    if (text == "or") return make(TokenKind::OrOr, start, begin, "||");
    if (text == "not") return make(TokenKind::Bang, start, begin, "!");

    const auto it = keywords.find(text);
    return make(it == keywords.end() ? TokenKind::Identifier : it->second, start, begin, std::move(text));
}
...
Token Lexer::string_literal() {
    ...
    if (!match('"')) diags_.error({begin, loc_}, "E0002", "unterminated string literal");
...
Token Lexer::char_literal() {
    ...
    if (!match('\'')) diags_.error({begin, loc_}, "E0004", "unterminated character literal");
...
std::vector<Token> Lexer::lex() {
    ...
    while (!eof()) {
        if (line_start_ && delimiter_depth_ == 0) {
            ...
            if (pos_ != before && line_start_) continue;
            if (eof()) break;
        }
        ...
        switch (c) {
            ...
            case ',':
                ...
                break;
            case '.':
                if (match('.')) kind = match('=') ? TokenKind::DotDotEq : TokenKind::DotDot;
            ...
            case '~':
                ...
                break;
            case '%':
                kind = match('=') ? TokenKind::PercentEq : TokenKind::Percent;
                break;
            case '^':
                kind = match('=') ? TokenKind::CaretEq : TokenKind::Caret;
                break;
            case ':':
                kind = match(':') ? TokenKind::ColonColon : TokenKind::Colon;
                break;
            case '-':
                kind = match('>') ? TokenKind::Arrow : (match('=') ? TokenKind::MinusEq : TokenKind::Minus);
                break;
            case '=':
                kind = match('=') ? TokenKind::EqEq : (match('>') ? TokenKind::FatArrow : TokenKind::Eq);
                break;
            case '+':
            ...
            case '&':
                ...
                break;
            case '|':
                if (match('|')) kind = TokenKind::OrOr;
                else kind = match('=') ? TokenKind::PipeEq : TokenKind::Pipe;
                break;
            case '!':
                kind = match('=') ? TokenKind::BangEq : TokenKind::Bang;
                break;
            case '<':
                if (match('<')) kind = match('=') ? TokenKind::ShiftLeftEq : TokenKind::ShiftLeft;
                else kind = match('=') ? TokenKind::LessEq : TokenKind::Less;
                break;
            case '>':
                if (match('>')) kind = match('=') ? TokenKind::ShiftRightEq : TokenKind::ShiftRight;
                else kind = match('=') ? TokenKind::GreaterEq : TokenKind::Greater;
                break;
            default:
                diags_.error({begin, loc_}, "E0005", std::string("unexpected character '") + c + "'");
                continue;
        }
        out.push_back(make(kind, start, begin));
    }

    if (!out.empty() && out.back().kind != TokenKind::Newline && out.back().kind != TokenKind::Dedent &&
        out.back().kind != TokenKind::Indent) {
        out.push_back({TokenKind::Newline, "\n", {loc_, loc_}});
    }
