...
Token Lexer::identifier_or_keyword() {
    ...
    static const std::unordered_map<std::string, TokenKind> keywords = {
        ...
        {"loop", TokenKind::KwLoop},       {"shared", TokenKind::KwShared},
        {"percpu", TokenKind::KwPerCpu},   {"concurrent", TokenKind::KwConcurrent},
...
std::vector<Token> Lexer::lex() {
    ...
    while (!eof()) {
        ...
        switch (c) {
            ...
            case '.':
                if (match('.'))
                    kind = match('=') ? TokenKind::DotDotEq : TokenKind::DotDot;
                else
                    kind = TokenKind::Dot;
                break;
            case ';':
            ...
            case '|':
                if (match('|'))
                    ...
                    kind = match('=') ? TokenKind::PipeEq : TokenKind::Pipe;
                break;
            case '!':
            ...
            case '<':
                if (match('<'))
                    kind = match('=') ? TokenKind::ShiftLeftEq : TokenKind::ShiftLeft;
                else
                    kind = match('=') ? TokenKind::LessEq : TokenKind::Less;
                break;
            case '>':
                if (match('>'))
                    kind = match('=') ? TokenKind::ShiftRightEq : TokenKind::ShiftRight;
                else
