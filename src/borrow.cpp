...
Type BorrowChecker::type_of_place(std::string_view p, const State& s) const {
    ...
    if (!found) {
        auto g = model_.globals.find(root);
...
void BorrowChecker::collect_uses(const ast::Stmt& s,
    ...
    if (auto l = dynamic_cast<const ast::LetStmt*>(&s)) {
        ...
        collect_expr_uses(*f->begin, i, o);
        collect_expr_uses(*f->end, i, o);
        for (auto& y : f->body->stmts)
            collect_uses(*y, i, o);
    } else if (auto loop = dynamic_cast<const ast::LoopStmt*>(&s)) {
        for (auto& y : loop->body->stmts)
            collect_uses(*y, i, o);
...
void BorrowChecker::check_stmt(const ast::Stmt& st, State& s) {
    ...
    if (auto f = dynamic_cast<const ast::ForStmt*>(&st)) {
        inspect_expr(*f->begin, s, Access::Move);
        inspect_expr(*f->end, s, Access::Move);
        State body = s;
        body.scopes.emplace_back();
        auto it = model_.expr_types.find(f->begin.get());
        body.scopes.back()[f->name] = it == model_.expr_types.end() ? Type{} : it->second;
        check_block(*f->body, body);
        return;
    }
    if (auto l = dynamic_cast<const ast::LoopStmt*>(&st)) {
        State body = s;
        check_block(*l->body, body);
        return;
    }
